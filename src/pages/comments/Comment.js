...
const Comment = (props) => {
    const { id, commentlike_id, commentlikes_count, profile_id, profile_image, owner, updated_on, content } = props;
    ...
    const handleLike = async () => {
      ...
      const handleUnlike = async () => {
        try {
          await axiosRes.delete(`/commentslikes/${commentlike_id}/`);
          setComment((prevComment) => ({
            ...prevComment,
            results: prevComment.results.map((comment) => {
              return comment.id === id
  ...
  return (
    <div>
        ...
        {is_owner ? (
          ...
          <OverlayTrigger
            placement="top"
            overlay={<Tooltip>Log in to like comment!</Tooltip>}
          >
            <i className="far fa-heart" />
          </OverlayTrigger>
        )}
